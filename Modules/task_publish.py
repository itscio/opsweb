...
from Modules import Md5,db_op,produce,loging,init
...
def _console_out(channel,Msg_key,Msg):
    ...
    if channel == 'web':
        Redis.lpush(Msg_key,'%s  %s' %(tm,Msg))
#结果汇总模块
...
def Publish_agent(args):
    ...
    #消息队列模块
    def Deploy_project(server_lists_key,ssh,K):
        while True:
            #部署异常，停止部署
    ...
    #代码分发控制逻辑
    try:
        server_lists_key, server_lists_counts, d_files,project,package_name, package_path, Msg_Key, K, INFOS = args
        ...
        try:
            #自动回滚失败Key
            rollback_fail_key = 'rollback_fail_%s' % K
            #自动回滚统计key
            rollback_count_key = 'rollback_count_%s' % K
            #部署成功Key
            deploy_success_key = 'deploy_success_%s' % K
            #部署失败Key
            deploy_fail_key = 'deploy_fail_%s' % K
            ...
            #代码分发
            result = Deploy_project(server_lists_key,ssh,K)
        ...
        if os.path.exists(d_files):
            ...
            if vals:
                ...
                for infos in server_lists:
                    ...
                    try:
                        ...
                        if not tcpping(host=ip, port=app_port, timeout=3):
                            ...
                            raise _console_out(channel,Msg_Key,_Msg)
                #部署列表写入redis队列
                server_lists_key = '%s_server_lists_key' %K
                ...
                for i in range(Publish_types[publish_type]):
                    ...
                    Scheduler = Scheduler.Scheduler_mem(Publish_agent, [server_lists_key,server_lists_counts,d_files,project,package_name,package_path,Msg_Key,K,INFOS])
                    Scheduler.start()
                    time.sleep(1)
            else:
