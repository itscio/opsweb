...
def scheduler_tasks():
    if HOST in task_servers:
        ...
        #scheduler.add_job(Task.influxdb_alarm, 'cron', second='0', minute=run_date[1], hour=run_date[0], id=Task.influxdb_alarm.__name__, replace_existing=True)
        #scheduler.add_job(Task2.make_redis_relation, 'cron', second='0', minute=run_date[1], hour=run_date[0], id=Task2.make_redis_relation.__name__,replace_existing=True)
        ################################################################################################################################################
